...
export async function GET(req: NextRequest) {
  try {
    ...
    const paymentId = order.order_id || "unknown";

    // Generate coupon
    await connectToDatabase();

    // Check if we ALREADY processed this exact order to prevent duplicate coupons
    const alreadyProcessed = await Coupon.findOne({ orderId: orderId });
    if (alreadyProcessed) {
      return NextResponse.json({ paid: true, coupon: alreadyProcessed });
    }
    ...
    const lunchDate = todayPolled
      ? todayPolled.validForDate
      : getNextLunchDate();
